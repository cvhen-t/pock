...
export class RanchSystem {
    ...
    refresh() {
        ...
        for (const stack of this.stacks.getAllStacks()) {
            ...
            if (animals > (effect.maxAnimals ?? 99))
                continue;
        ...
        for (const stackId of this.active.keys()) {
            if (!needed.has(stackId))
                this.stopRanch(stackId);
        }
    }
    startTimer(stack, effect, mode) {
...
function getRanchEffect(def) {
    return def.effects?.find((e) => e.type === 'ranch_pen');
}
export function getRanchPenEffect(def) {
    return getRanchEffect(def);
}
function countAnimals(stack, effect) {
...
function countByTag(stack, tag) {
    return stack.members.filter((m) => (m.definition.tags ?? []).includes(tag)).length;
}
function cardMatchesFeed(card, req) {
    const tags = card.definition.tags ?? [];
    if (req.cardId && card.definition.id === req.cardId)
        return true;
    if (req.tag && tags.includes(req.tag))
        return true;
    return false;
}
function hasFeed(stack, requirements) {
...
function consumeFeed(stack, requirements) {
    const toRemove = [];
    const pool = [...stack.members];
    for (const req of requirements) {
        let needed = req.count;
