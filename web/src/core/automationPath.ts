...
export function sortHandHasMatchingDownstream(
...
export function isSourceRelayPathActive(
  graph: AutomationGraph,
  sourceDev: LogisticsDevice,
): boolean {
  const relayEdge = graph.edges.find(
    (e) => e.from.id === sourceDev.id && e.toRole === 'auto_relay',
  );
...
export function getRelayDeviceForSource(
  graph: AutomationGraph,
  sourceDev: LogisticsDevice,
): LogisticsDevice | undefined {
  return graph.edges.find((e) => e.from.id === sourceDev.id && e.toRole === 'auto_relay')?.to;
}

export function getRelayDeviceForSortHand(
  graph: AutomationGraph,
  sortHand: GameCard,
): LogisticsDevice | undefined {
  return graph.edges.find(
    (e) => e.to.card === sortHand && e.fromRole === 'auto_relay' && e.toRole === 'logistics_sorter',
  )?.from;
}

export function listSourcesOnRelay(
  graph: AutomationGraph,
  relayDev: LogisticsDevice,
): LogisticsDevice[] {
  return graph.edges
    .filter(
      (e) =>
        e.to.id === relayDev.id &&
        e.toRole === 'auto_relay' &&
        SOURCE_ROLES.includes(e.from.role as SourceRole),
    )
    .map((e) => e.from);
}

export function isAutomationEdgeActive(
  graph: AutomationGraph,
  edge: AutomationEdge,
): boolean {
  ...
  if (
    edge.toRole === 'auto_relay' &&
    SOURCE_ROLES.includes(edge.from.role as SourceRole)
  ) {
    return isSourceRelayPathActive(graph, edge.from);
  }
  if (edge.from.role === 'auto_relay' && edge.toRole === 'logistics_sorter') {
    const hasActiveSource = listSourcesOnRelay(graph, edge.from).some((src) =>
      isSourceRelayPathActive(graph, src),
    );
...
export function isActiveCollectorChain(
  ...
  return (
    collectorDevice.role === 'logistics_collect' &&
...
export function isActiveFacilityRelayChain(
  ...
  return (
    facilityDevice.role === 'logistics_facility' &&
...
export function isActiveWarehouseRelayChain(
  ...
  return (
    warehouseDevice.role === 'warehouse' && isSourceRelayPathActive(graph, warehouseDevice)
  );
}

export type SortHandPathStatus =
  | 'ok'
  | 'no_downstream'
  | 'no_relay'
  | 'no_matching_source';

export function getSortHandPathStatus(
  graph: AutomationGraph | undefined,
  sortHand: GameCard,
): SortHandPathStatus {
  if (!graph) return 'no_relay';
  if (!sortHandHasMatchingDownstream(graph, sortHand)) return 'no_downstream';
  const relay = getRelayDeviceForSortHand(graph, sortHand);
  if (!relay) return 'no_relay';
  const mode = getSortMode(sortHand);
  const hasSource = listSourcesOnRelay(graph, relay).some((src) => {
    if (!isSourceRelayPathActive(graph, src)) return false;
    if (mode === 'buy') return src.role === 'logistics_collect';
  ...
  if (mode === 'sell' || mode === 'store') {
    const storageOnRelay = listSourcesOnRelay(graph, relay).filter(
      (s) => s.role === 'warehouse' || s.role === 'logistics_depot',
    );
    if (storageOnRelay.length === 0) return 'no_matching_source';
    return hasSource ? 'ok' : 'no_matching_source';
  }
  return hasSource ? 'ok' : 'no_matching_source';
}

export function sortHandPathHintSubtitle(status: SortHandPathStatus, mode: string): string {
  switch (status) {
    case 'no_downstream':
