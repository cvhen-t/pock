...
import { buildProximityEdges, buildProximityEdgesStable, diffAutomationEdges, } from './automationNetworkEdges';
...
const RADIUS = 220;
function mockDevice(id, role, x, y, sortMode) {
    const data = new Map();
    if (sortMode)
        data.set('sortMode', sortMode);
    return {
        id,
        role,
        tags: [role],
...
describe('buildProximityEdgesStable', () => {
    ...
    it('mover 更近时可抢占传送器入边', () => {
        ...
        const devices = [colA, colB, relay];
        const prev = buildProximityEdgesStable([colA, mockDevice('colB', 'logistics_collect', 300, 0), relay], LINK_RULES, RADIUS);
        expect(edgeKeys(prev)).toEqual(['colA→relay']);
        const next = buildProximityEdgesStable(devices, LINK_RULES, RADIUS, {
            prevEdges: prev,
    ...
    it('mover 传送器更近时可抢占分拣手入边', () => {
        ...
        const devices = [relayA, relayB, sorter];
        const prev = buildProximityEdgesStable([relayA, mockDevice('relayB', 'auto_relay', 300, 0), sorter], LINK_RULES, RADIUS);
        expect(edgeKeys(prev)).toEqual(['relayA→sorter']);
        const next = buildProximityEdgesStable(devices, LINK_RULES, RADIUS, {
            prevEdges: prev,
