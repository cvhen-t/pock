...
export function describeStackDrop(stacks, dragged, target) {
    if (!stacks.wouldAcceptStack(dragged, target))
        return null;
    ...
    if (dragTags.includes('survivor') && baseTags.includes('worksite')) {
        ...
        if (effect?.outputCardId) {
            const outName = dataStore.getCard(effect.outputCardId)?.name ?? effect.outputCardId;
    ...
    if (stationId && baseTags.includes('craft_station')) {
        ...
        const currentDay = 1;
        const recipe = previewFacilityRecipe(stationId, members, dataStore.getRecipes(), currentDay);
        if (recipe) {
            const outName = recipe.output.cardId === '_same_seed'
                ? '同种种子'
    ...
    if (dragTags.includes('weapon')) {
        const pile = [targetStack.base, ...targetStack.members];
...
function getShelterEffect(def) {
    return def.effects?.find((e) => e.type === 'shelter');
}
function getBaseCoreEffect(def) {
    return def.effects?.find((e) => e.type === 'base_core');
}
