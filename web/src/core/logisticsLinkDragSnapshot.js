...
import { buildProximityEdgesStable, deviceDist, diffAutomationEdges, } from './automationNetworkEdges';
...
function edgeTouchesMover(e, moverIds) {
    return moverIds.has(e.from.id) || moverIds.has(e.to.id);
}
function edgeToPreviewLink(e, moverIds) {
...
function isConnected(nextEdges, fromId, toId) {
    return nextEdges.some((e) => e.from.id === fromId && e.to.id === toId);
}
function findBlockedLinks(devices, moverIds, linkRadius, rules, nextEdges, dragOpts) {
    const blocked = [];
        ...
        for (const rule of rules) {
            ...
            if (mover.role === rule.from) {
                for (const to of devices.filter((d) => d.role === rule.to && d.id !== mover.id)) {
                    ...
                    if (d > linkRadius)
                        continue;
                    if (isConnected(nextEdges, mover.id, to.id))
                        continue;
                    const incoming = nextEdges.filter((e) => e.to.id === to.id && e.fromRole === rule.from && e.toRole === rule.to);
            ...
            if (mover.role === rule.to) {
                for (const from of devices.filter((d) => d.role === rule.from && d.id !== mover.id)) {
                    ...
                    if (d > linkRadius)
                        continue;
                    if (isConnected(nextEdges, from.id, mover.id))
                        continue;
                    const incoming = nextEdges.filter((e) => e.to.id === mover.id && e.fromRole === rule.from && e.toRole === rule.to);
                    if (incoming.length >= rule.maxIn) {
...
function nearestOutOfRangePartner(devices, mover, rules, linkRadius, dragOpts) {
    let best = null;
    for (const rule of rules) {
        if (rule.from === 'logistics_sorter')
            continue;
        if (mover.role === rule.from) {
            for (const to of devices.filter((d) => d.role === rule.to && d.id !== mover.id)) {
                ...
                if (d <= linkRadius)
                    continue;
                if (!best || d < best.d)
                    best = { partner: to, toRole: rule.to, d };
            }
        }
    }
    if (!best)
...
function buildDragHint(snapshot, visual, anchor, linkRadius, devices, rules, dragOpts) {
...
export function computeLogisticsDragSnapshot(scene, anchor, linkRadius, dragCards, visual, rules) {
    ...
    const active = nextEdges
        .filter((e) => edgeTouchesMover(e, moverIds))
        .map((e) => edgeToPreviewLink(e, moverIds));
    const blocked = findBlockedLinks(devices, moverIds, linkRadius, linkRules, nextEdges, dragOpts);
