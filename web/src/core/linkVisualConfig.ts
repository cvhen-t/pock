export interface LinkHintTemplates {
  willConnect: string;
  willDisconnect: string;
  slotTaken: string;
  tooFar: string;
  inRangeConnect: string;
}
...
export interface LinkVisualConfig {
  ...
  breakEdgeColor: string;
  inactiveAlpha: number;
  previewAlpha: number;
  breakAlpha: number;
  blockedCandidateAlpha: number;
  ...
  linkHints: LinkHintTemplates;
}

const DEFAULT_LINK_HINTS: LinkHintTemplates = {
...
export function parseLinkVisual(raw: Record<string, unknown> | undefined): LinkVisualConfig {
  const hints = (raw?.linkHints as Record<string, string> | undefined) ?? {};
  return {
...
/** 替换 `{key}` 占位符 */
export function formatLinkHint(template: string, vars: Record<string, string>): string {
  let out = template;
