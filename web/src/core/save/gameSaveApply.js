...
function restoreBoardStack(ctx, saved) {
    const base = spawnSnapshotCard(ctx, saved.base);
    if (!base)
        return;
    base.stackId = saved.stackId;
    const stack = { id: saved.stackId, base, members: [] };
...
function restoreBlueprintStack(ctx, saved) {
    const base = spawnSnapshotCard(ctx, saved.base);
    if (!base)
        return;
...
export function applySaveFile(ctx, save) {
