...
export class LogisticsRangePreview {
    ...
    show(worldX, worldY, spec, snapshot) {
        this.g.clear();
        ...
        for (const link of snapshot.blocked) {
            if (addedOthers.has(link.other))
                continue;
            this.drawLink(link, visual, hexToNumber(visual?.blockedColor ?? '#b04040'), 2, blockedAlpha, true, dashLen, dashGap);
            this.highlightCandidate(link.other, BLOCKED_STROKE, blockedAlpha * 0.5);
        }
    }
    drawLink(link, visual, overrideColor, width, alpha, dashed, dashLen, dashGap) {
        const key = edgeStyleKey(link.fromRole, link.toRole);
        const style = visual?.edgeStyles[key];
        const color = overrideColor ?? hexToNumber(style?.color ?? visual?.previewEdgeColor ?? '#5a8ac8');
    ...
    highlightCandidate(card, stroke, fillAlpha) {
