...
export type HudActionKey = 'speed' | 'settings' | 'guide';
...
export default class TopHud extends Phaser.GameObjects.Container {
  ...
  private bottomEdge!: Phaser.GameObjects.Rectangle;

  private leftRail!: Phaser.GameObjects.Container;

  private centerRail!: Phaser.GameObjects.Container;

  private rightRail!: Phaser.GameObjects.Container;

  private resourceChips: ResourceChip[] = [];

  private dayLabel!: Phaser.GameObjects.Text;

  private dayTimer!: Phaser.GameObjects.Text;

  private dayBarBg!: Phaser.GameObjects.Rectangle;

  private dayBarFill!: Phaser.GameObjects.Rectangle;

  private basePanel!: Phaser.GameObjects.Container;

  private baseLabel!: Phaser.GameObjects.Text;

  private baseBarBg!: Phaser.GameObjects.Rectangle;

  private baseBarFill!: Phaser.GameObjects.Rectangle;

  private actionRail!: Phaser.GameObjects.Container;

  private speedBtn!: HudActionButton;

  private guideBtn!: HudActionButton;

  private settingsBtn!: HudActionButton;

  private dayEvent?: Phaser.Time.TimerEvent;
  ...
  private buildResourceChips(): void {
    const keys: (keyof ResourceSnapshot)[] = ['food', 'water', 'caps'];
  ...
  private buildDaySection(): void {
    ...
    this.dayBarFill = this.scene.add.rectangle(
      -HUD_DAY_BAR_W / 2,
      0,
      HUD_DAY_BAR_W,
      4,
  ...
  private buildActionRail(): void {
    this.actionRail = this.scene.add.container(0, 0);

    this.speedBtn = new HudActionButton(this.scene, 0, 0, '×1', () => {
      this.cycleSpeed();
      this.scene.events.emit('hud-action', { key: 'speed' as HudActionKey, speed: this.speedLevel });
    });

    this.guideBtn = new HudActionButton(
      this.scene,
      0,
      0,
      '图鉴',
      () => {
        this.scene.events.emit('hud-action', { key: 'guide' as HudActionKey });
      },
      HUD_GUIDE_BTN,
    );
  ...
  private resolveIcon(cardId: string): string {
    const key = TEX.icon(cardId);
    if (this.scene.textures.exists(key)) return key;
    return TEX.icon('caps');
  }

  applyLayout(centerX: number, centerY: number, screenWidth: number, totalHeight = HUD_BAR_H): void {
    this.setPosition(centerX, centerY);
    this.layoutBar(screenWidth, totalHeight);
  }

  private layoutBar(screenWidth: number, totalHeight: number): void {
    ...
    this.speedBtn.setPosition(
      -HUD_ACTION_BTN - HUD_ACTION_GAP - HUD_GUIDE_BTN - HUD_ACTION_GAP - HUD_ACTION_BTN / 2,
      0,
    );
