...
import type { MutantGrowthSystem } from '../systems/MutantGrowthSystem';
import type { WorkSiteSystem } from '../systems/WorkSiteSystem';
...
export default class StackLane extends Phaser.GameObjects.Container {
  ...
  private entries = new Map<string, LaneEntry>();
  ...
  constructor(
    ...
    private readonly workSites: WorkSiteSystem,
    private readonly mutantGrowth: MutantGrowthSystem,
  ) {
    super(scene, 0, 0);
  ...
  private refresh = (): void => {
    ...
    for (const stack of tracked.slice(0, STACK_LANE_MAX_ENTRIES)) {
      ...
      container.setInteractive(
        new Phaser.Geom.Rectangle(-laneW / 2 + 4, -ENTRY_H / 2, laneW - 8, ENTRY_H),
  ...
  private buildEntry(stack: CardStack, laneW: number): Phaser.GameObjects.Container {
  ...
  private describeStack(stack: CardStack): { label: string; seconds: number | null } {
    const baseTags = stack.base.definition.tags ?? [];
    const hasSeed = stack.members.some((m) =>
      (m.definition.tags ?? []).includes('mutant_seed'),
    );
  ...
  private focusStack(stack: CardStack): void {
...
function getSpawnEffect(def: { effects?: { type: string }[] }): SpawnTimerEffect | undefined {
  return def.effects?.find((e) => e.type === 'spawn_timer') as SpawnTimerEffect | undefined;
}

function stackHasSurvivor(stack: CardStack): boolean {
  return [stack.base, ...stack.members].some((c) =>
    (c.definition.tags ?? []).includes('survivor'),
  );
}
